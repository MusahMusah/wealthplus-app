...
if (!function_exists('kyc_required')) {
    ...
    {
        $isRequire = (kyc_enabled() && gss('kyc_before_' . $type, 'no') == 'yes')  ? true : false;

        if ($user === true) {
            $authUser = auth()->user();

            return ($isRequire && !$authUser->kyc_verified) ? true : false;
        }
        return $isRequire;
    }
}
...
if (!function_exists('profile_lockable')) {
    ...
    {
        if (auth()->check()) {
            return (auth()->user()->kyc_verified && gss('kyc_locked_profile', 'yes') == 'yes') ? true : false;
        }
