...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/'.$style.'.css?ver=115') }}">
@if(sys_settings('ui_theme_skin', 'default')!='default')
    <link rel="stylesheet" href="{{ asset('assets/css/skins/theme-'.sys_settings('ui_theme_skin').'.css?ver=115') }}">
@endif
...
<script type="text/javascript">
    const updateSetting = "{{ route('update.setting') }}", getTnxDetails = "{{ route('transaction.details') }}", msgwng = "{{ __("Sorry, something went wrong!") }}", msgunp = "{{ __("Unable to process your request.") }}"{!! (user_consent() === null) ? ', consentURI = "'.route('gdpr.cookie').'"' : '' !!};
</script>
<script src="{{ asset('/assets/js/bundle.js?ver=115') }}"></script>
<script src="{{ asset('/assets/js/app.js?ver=115') }}"></script>
<script src="{{ asset('/assets/js/charts.js?ver=115') }}"></script>
@stack('scripts')
