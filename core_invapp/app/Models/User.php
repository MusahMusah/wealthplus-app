...
class User extends Authenticatable
{
     ...
     * @since 1.1.2
     */
    public function getEligibleForKycAttribute()
    {
        if (!$this->has_basic) return false;

        if (gss('kyc_complete_profile', 'no') == 'no') {
            return true;
        }

        $eligible = true;
        $kyc_metas = gss('kyc_required_metas', []);

        foreach ($kyc_metas as $key => $value) {
